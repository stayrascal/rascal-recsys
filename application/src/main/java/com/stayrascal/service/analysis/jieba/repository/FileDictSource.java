...
import static com.stayrascal.service.analysis.jieba.WordDictionary.USER_DICT_SUFFIX;
...
public class FileDictSource implements DictSource {
    ...
    public void loadDict(Charset charset, Consumer<String[]> consumer) throws IOException {
        ...
        if (Files.isDirectory(dictPath)) {
            DirectoryStream<Path> stream = Files.newDirectoryStream(dictPath, String.format(Locale.getDefault(), "*%s", USER_DICT_SUFFIX));
