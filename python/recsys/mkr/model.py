...
class MKR(object):
    ...
    def _build_low_layers(self, args):
        ...
        for _ in range(args.L):
            ...
            self.user_emb_matrix = user_mlp(self.user_embeddings)
    ...
    def _build_high_layers(self, args):
        # RS
        use_inner_produce = True
        if use_inner_produce:
            self.scores = tf.reduce_sum(self.user_embeddings * self.item_embeddings, axis=1)
        else:
            self.user_item_concat = tf.concat([self.user_emb_matrix, self.item_embeddings], axis=1)
        ...
        self.score_kge = tf.nn.sigmoid(tf.reduce_sum(self.tail_embeddings * self.tail_pred, axis=1))
    ...
    def _build_loss(self, args):
        ...
        # KGE
        self.base_loss_kge = -self.score_kge
    ...
    def train_rs(self, sess, feed_dict):
        return sess.run([self.optimizer_rs, self.loss_rs], feed_dict=feed_dict)

    def train_kge(self, sess, feed_dict):
        return sess.run([self.optimizer_kge, self.loss_kge], feed_dict=feed_dict)

    def eval(self, sess, feed_dict):
        labels, scores = sess.run([self.labels, self.scores_normalized], feed_dict=feed_dict)
    ...
    def get_scores(self, sess, feed_dict):
        return sess.run([self.item_indices, self.scores_normalized], feed_dict=feed_dict)
